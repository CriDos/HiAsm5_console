...
public:
    ...
    ustring serialize()
    {
        return DataValue::serialize(type, value);
    }
    /**
    ...
    int readInt()
    {
        return (type == data_int) || (type == data_combo) || (type == data_comboEx) ? *(int *)value : 0;
    }
    /**
    ...
    double readReal()
    {
        return type == data_real ? *(double *)value : 0;
    }
    /**
    ...
    ustring readStr()
    {
        return (type == data_str) || (type == data_list) || (type == data_stock) || (type == data_element) ? *(ustring *)value : ustring();
    }
    /**
    ...
    TData *readData()
    {
        return type == data_data ? (TData *)value : NULL;
    }
...
typedef struct _ArrayValueItem {
    ustring name;
    void *data;
    _ArrayValueItem(ustring name, void *data)
    {
        this->name = name;
        this->data = data;
    }

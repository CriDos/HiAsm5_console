...
void ElementCoreDSC_QueryScalar::do_work(ElementPoint *point, TData *data)
{
    if(sqlite) {
        ...
        if(sqlite3_data_count(r)) {
            ...
            case SQLITE_INTEGER:
                result = sqlite3_column_int(r, 0);
                break;
            case SQLITE_FLOAT:
                result = sqlite3_column_double(r, 0);
                break;
            default:
    ...
    if(sqlite) {
        ...
        while(sqlite3_step(r) != SQLITE_DONE) {
            ...
            for(int i = 0; i < sqlite3_data_count(r); i++) {
                ...
                case SQLITE_INTEGER:
                    dt << sqlite3_column_int(r, i);
                    break;
                case SQLITE_FLOAT:
                    dt << sqlite3_column_double(r, i);
                    break;
                default:
...
void ElementCoreConvertor::do_work(ElementPoint *point, TData *data)
{
    ...
    case 4:
        result = ustring(1, (char)dt.toInt());
        break;
    case 5:
        break;  //hextoint
    case 6: {
...
Widget *ElementTplButtonBox::createWidget(Element **parent)
{
    *parent = this;
    ...
    if(tpl->conf->cClass == "HCWE_VButtonBox")
        return new VButtonBox((ButtonBoxStyle)(props[8]->readInt() + BUTTONBOX_DEFAULT_STYLE), props[7]->readInt());
    return new HButtonBox((ButtonBoxStyle)(props[8]->readInt() + BUTTONBOX_DEFAULT_STYLE), props[7]->readInt());
}
...
{
    adj = new Adjustment(props[9]->readReal(), props[7]->readReal(), props[8]->readReal());
    Scrollbar *sb;
...
Widget *ElementTplColorButton::createWidget(Element **parent)
{
    ...
    cb->set_color(*(Gdk::Color *)props[7]->value);
...
void ElementTplColorButton::do_work(ElementPoint *point, TData *data)
{
    if(point->name == "doColor") {
        ((ColorButton *)ctl)->set_color(Gdk::Color(data ? data->toStr() : "black"));
    } else
        ElementTplWinControl::do_work(point, data);
}
...
Widget *ElementTplProgressBar::createWidget(Element **parent)
{
    ...
    pb->set_orientation((ProgressBarOrientation)(props[8]->readInt() + PROGRESS_LEFT_TO_RIGHT));
    return pb;
}
...
{
    if(point == points[0]) {
        ((ComboBoxText *)ctl)->append_text(data ? data->toStr() : "");
    } else if(point->name == "doClear")
        ((ComboBoxText *)ctl)->clear_items();

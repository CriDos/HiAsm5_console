...
int propToInteger(ElementProperty *prop)
{
    ...
    if(prop->type == data_color) {
        Gdk::Color &c = *(Gdk::Color *)prop->value;

        return (c.get_red() >> 8) | ((c.get_green() >> 8) << 8) | ((c.get_blue() >> 8) << 16);
    }
    return *(int *)prop->value;
}
...
int GetParam(short index, void *value)
{
    ...
    */
    ...
    case PARAM_DEBUG_SERVER_PORT:
        *(int *)value = 100;
        return 0; //glbParams.readInt(GP_dbg_ServerPort);
    case PARAM_DEBUG_CLIENT_PORT:
        *(int *)value = 101;
        return 0; //glbParams.readInt(GP_dbg_ClientPort);
    ...
    case PARAM_USER_NAME:
        return 0; //ReturnParam(GP_usr_Name);
    case PARAM_USER_MAIL:
        return 0; //ReturnParam(GP_usr_Mail);
    case PARAM_PROJECT_NAME:
        //TODO TROLOLO ;D
        sdk = (*(Element **)value)->parent->getMSDK();
        s = Glib::filename_display_basename(sdk->fileName);
        s.erase(s.length() - 4, 4);   // remove .sha
        strcpy((char *)value, s.c_str());
        return 0;
...
ElementProperty prop(NULL, ustring(), ustring(), data_int);

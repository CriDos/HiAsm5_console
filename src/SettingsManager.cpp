...
void SettingsManager::fillProps(int pid, PropertyItem *group)
{
    ...
    while(mdb.read_settings(id, param)) {
        ...
        switch(param.type) {
        case 1:
            p = new PropertyInteger(param.title, tmp);
            break;
        case 2:
            p = new PropertyDouble(param.title, tmp);
            break;
        case 3:
            p = new PropertyString(param.title, tmp);
            break;
        case 4:
            p = new PropertyColor(param.title, tmp);
            break;
        case 5:
            p = new PropertyEnum(param.title, tmp, param.list);
            break;
        case 0:
...
int SettingsManager::readInt(SettingsIDS id)
{
    return *(int *)values[id];
}
double SettingsManager::readReal(SettingsIDS id)
{
    return *(double *)values[id];
}
ustring &SettingsManager::readStr(SettingsIDS id)
{
    return *(ustring *)values[id];
}
Gdk::Color SettingsManager::readColor(SettingsIDS id)
{
    return *(Gdk::Color *)values[id];
}

...
ustring TData::toStr()
{
    switch(type) {
    case data_int:
        return int_to_str(idata);
    case data_str:
        return sdata;
    case data_real:
        return double_to_str(rdata);
    default:
        return ustring();
    }
}
...
bool TData::operator == (const _TData &data)
{
    if(type == data.type)
        switch(type) {
        case data_null:
            return data.type == data_null;
        case data_int:
            return data.idata == idata;
        case data_real:
            return data.rdata == rdata;
        case data_str:
            return data.sdata == sdata;
        default:
            return false;
        }
    return false;
...
bool TData::compare(_TData &data)
{
    ...
    case data_null:
        return data.empty();
    case data_int:
        return idata == data.toInt();
    case data_real:
        return rdata == data.toReal();
    case data_str:
        return sdata == data.toStr();
    default:
        return false;
    }
}
